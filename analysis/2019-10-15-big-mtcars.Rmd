...
vans <-  big_epa_cars %>%
  ...
  mutate(name = str_c(make, model, year, sep = "-"),
         hybrid = if_else(str_detect(atvType, "[Hh]ybrid"),
                          "hybrid", "ICE", missing = "ICE")) %>%
  select(name, make, model, year, VClass, youSaveSpend, ghgScore, hybrid)
```


```{r vans}
ggplot(vans, aes(x = youSaveSpend, y = ghgScore)) +
  geom_text(aes(label = make, color = hybrid), size = 5,
